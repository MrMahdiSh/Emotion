...
const ExportModal: React.FC<ExportModalProps> = ({
  ...
  const [rangeType, setRangeType] = useState<"7d" | "30d" | "all" | "custom">(
    "30d",
  );
  ...
  const handleExport = async () => {
    ...
    if (rangeType === "7d") {
      ...
      filteredEntries = entries.filter((e) =>
        isAfter(new Date(e.date), cutoff),
      );
    ...
    try {
      // 1. Check for the .NET 9 HybridWebView Bridge FIRST
      // @ts-ignore
      if (window.HybridWebView) {
        const payload = {
          Type: "FileExport",
          FileName: `${fileName}.json`, // Ensure extension is passed if C# expects it
          Content: dataStr,
        };
        ...
        // 2. Browser Fallback (Only if HybridWebView is missing)
        console.warn(
          "HybridWebView not found - falling back to browser download",
        );
          ...
          <div className="space-y-3 mb-8">
            ...
            {rangeType === "custom" && (
              <div className="grid grid-cols-2 gap-3 mt-2 animate-fade-in">
                <div>
                  <label className="text-xs text-textMain/50 block mb-1">
                    {t("startDate")}
                  </label>
                  <input
                    type="date"
